...
show_main_menu() {
    ...
    green "=========================================="
    echo ""
    blue "当前域名: ${yellow}${CURRENT_DOMAIN}${re}"
    echo ""
    echo "请选择操作："
    echo ""
    echo "  ${green}1${re}) 📦 创建本地备份"
    echo "  ${green}2${re}) 💙 备份到 GitHub"
    echo "  ${green}3${re}) 🔄 恢复本地备份"
    echo "  ${green}4${re}) 🔄 从 GitHub 恢复"
    echo "  ${green}5${re}) 📋 查看备份列表"
    echo "  ${green}6${re}) ⚙️  GitHub 配置"
    echo "  ${green}0${re}) 🚪 退出"
    echo ""
}
